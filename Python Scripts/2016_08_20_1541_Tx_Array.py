# -*- coding: utf-8 -*-
"""
Created on Sat Aug 20 15:40:34 2016
...
def CreateCircle(name, center, r,unit = 'mm', WhichAxis ='Z'):
	...
	[
		...
		"IsCovered:="		, True,
		"XCenter:="		, [str(center[0]) + unit if type(center[0]) != type('a') else center[0]][0],
		"YCenter:="		, [str(center[0]) + unit if type(center[0]) != type('a') else center[0]][0],
		"ZCenter:="		, [str(center[0]) + unit if type(center[0]) != type('a') else center[0]][0],
		"Radius:="		, [str(r) + unit if type(r) != type('a') else r][0],
		"WhichAxis:="		, WhichAxis,
		"NumSegments:="		, "0"
	], 
...
for i in range(N):
    for j in range(N):
        ...
        r_in = P2[i][j]
        r_out = r_in + g + w
        name_bo = 'Circle_Big_o' + str(i) + str(j)
        name_bi = 'Circle_Big_i' + str(i) + str(j)
        name_so = 'Circle_Small_o' + str(i) + str(j)
        name_si = 'Circle_Small_i' + str(i) + str(j)
        

        CreateCircle(name_bo, center, r_out + w, WhichAxis ='Z')
        CreateCircle(name_bi, center, r_out , WhichAxis ='Z')
        CreateCircle(name_so, center, r_in + w, WhichAxis ='Z')
        CreateCircle(name_si, center, r_in, WhichAxis ='Z')

        Substract(name_bo, name_bi )
        Rename(name_bo, 'Ring_O'+str(i)+str(j))
        Substract(name_so, name_si )
        Rename(name_so, 'Ring_I'+str(i)+str(j))
        ...
        CreateArcSector(pl1, (0,0,0),('P/2',0,0), angle_off/2)
        CreateArcSector(pl2, (0,0,0),('P/2',0,0), -angle_off/2)
        CreateArcSector(pl3, (0,0,0),('-P/2',0,0), angle_off/2)
        CreateArcSector(pl4, (0,0,0),('-P/2',0,0), -angle_off/2)
        Unite([pl1,pl2,pl3,pl4])
        Substract('Ring_O'+str(i)+str(j), pl1, True )
        Substract('Ring_I'+str(i)+str(j), pl1)
