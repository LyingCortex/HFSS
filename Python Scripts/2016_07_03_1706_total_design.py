# -*- coding: utf-8 -*-
"""
Created on Sun Jul  3 17:04:19 2016
...
###==============================================================
#list_siw= [('-a/2','rad+w*'+str(i), 0 ) for i in range(N2+1)]    
...
pos = []
for i in range(N1+1):
    pos.append((-r1[i],z1[i]+0.4,0))

    
name = ['hvia_curve' + str(i+1) + 'sin' for i in range(N1+1)]
...
pos2 = []
for i in range(N2):
    pos2.append((-r2[i],z2[i]+0.4,0))

name2 = ['gauss' + str(i+1) + 'sin' for i in range(N2)]
###===============

local_var_array = get_design_varibles()
varible_value = [('L1',L1),('LL',LL),('rL',rL),('Ra', Ra),('ri',ri)]
...
r= 'rad'
for i in range(N1+1):
    CreateCylinder(pos[i], r, h, name[i], material, WhichAxis = 'Z', SolveInside = True, Transparency=0.7)
for i in range(N2):
    CreateCylinder(pos2[i], r, h, name2[i], material, WhichAxis = 'Z', SolveInside = True, Transparency=0.7)
