# -*- coding: utf-8 -*-
"""
Created on Mon Jun 27 16:07:02 2016
...
##the design varibles used in the design
local_var_array = get_design_varibles()
varible_value = [('D_air', D_air),('L_air', L_air),('d_air1', d_air1), ('d_air2', d_air2), ('d_air3', d_air3),('d_air4', d_air4),('off_y',off_y),('off_z',off_z),('off_left',off_left)]
Add_varible_list(varible_value)
varible_value_without_unit = [('Mz',Mz),('My',My)]
Add_varible_list_without_unit(varible_value_without_unit) 



#pos_air_2 = [( '0', '0', 'L1+L2+(L3-L_air)/2 + L_air/6'+ '*' + str((i+1)*2) + '-L_air/6/2') for i  in range(3)]
#pos_air_1 = [('0','L_air/6/2', 'L1+L2+(L3-L_air)/2 + L_air/6'+ '*' + str(2*i+1) + '-L_air/6/2') for i  in range(3)]
#pos_air=[]
#for i in range(3):
#    pos_air.append(pos_air_1[i])
#    pos_air.append(pos_air_2[i])
#
#name_air = [str(i+1) +  '_airCylinder' for i in range(6)]
#material = 'RF45'
#r = ['d_air1/2','d_air1/2', 'd_air2/2' , 'd_air2/2' , 'd_air3/2', 'd_air3/2'  ] 
#h= 'b'
#for i in range(6):
#    CreateCylinder(pos_air[i], r[i], h, name_air[i], material, WhichAxis = 'X', SolveInside = True, Transparency=0.7)
#    if (i%2 != 0):
#        DuplicateAlongLine(name_air[i], (0,2,0), 5, unit = 'mm')
#        DuplicateAlongLine(name_air[i], (0,-2,0), 5, unit = 'mm')
#    else:
#        DuplicateAlongLine(name_air[i], (0,2,0), 4, unit = 'mm')
#        DuplicateAlongLine(name_air[i], (0,-2,0), 5, unit = 'mm')
######################################################################################



pos_diameter1 = [[(0, '-off_y/2','L1+L2+ off_left+ off_z/2+off_z*'+str(i) ),'d_air'+str(i)+'/2'] for i in range(1,Mz)]
pos_diameter2 = [[(0, '-off_y*1.5', 'L1+L2+ off_left+ off_z/2+off_z*'+str(i)),'d_air'+str(i+1)+'/2'] for i in range(Mz)]
pos_diameter3 = [[(0, '-off_y*2.5', 'L1+L2+ off_left+ off_z/2+off_z*'+str(i)),'d_air'+str(i+1)+'/2'] for i in range(Mz)]

pos_diameter = []
for i in range(round(My/2)):
    pos_diameter.extend(locals()['pos_diameter'+str(i+1)])

name = ['air'+str(i+1) for i in range(Mz*3-1)]

h= 'b'
material = 'pec'

for i in range(Mz*3-1):
    CreateCylinder(pos_diameter[i][0], pos_diameter[i][1], h, name[i], material, WhichAxis = 'X', SolveInside = True, Transparency=0.7)
