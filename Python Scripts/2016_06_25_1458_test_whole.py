# -*- coding: utf-8 -*-
"""
Created on Sat Jun 25 14:58:09 2016
...
################### the design varibles used in the design
local_var_array = get_design_varibles()
varible_value = [('wp1',wp1),('wp2',wp2),('ll1',ll1),('ll2',ll2),('lsiw',lsiw),('w',w)]
...
list_siw = [('-a/2', 'll2+w*'+str(i), 0)  for i in range(1, N6+1)]
...
list_via_name = ['eedvia_f_'+ str(i+1) for i in range(length_via)]    
list_sum = [(z, x, y+'-ll2-lsiw-ll1-d/2') for (x, y, z) in list_total]
...
list_siw_new = [(z, x, y+'-ll2-lsiw-d/2') for (x, y, z) in list_siw]

name_siw = ['f_siw_via'+str(i+1) for i in range(len(list_siw))]
for i in range(len(list_siw)):
    CreateCylinder(list_siw_new[i], r, h, name_siw[i], material, WhichAxis = 'X', SolveInside = True, Transparency=0.7)
