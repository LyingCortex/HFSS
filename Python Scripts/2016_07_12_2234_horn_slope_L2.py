# -*- coding: utf-8 -*-
"""
Created on Tue Jul 12 22:34:26 2016
...
N1 = math.ceil(a/p)+1
N2 = math.ceil(L1/p)+1
N3 = math.ceil((L2-d/2)/p)+1

varible_value = [('p',p),('L1',L1),('a',a),('L2',L2)]
Add_varible_list(varible_value)
varible_value_without_unit = [('N1',N1), ('N2',N2),('N3',N3)]
Add_varible_list_without_unit(varible_value_without_unit)

## pos
pos1 = [('-a/2+p*'+str(i), 0, 0) for i in range(N1)]
pos2 = [('-a/2', 'p*'+str(i), 0) for i in range(1,N2)]
pos3 = [('a/2', 'p*'+str(i), 0) for i in range(1,N2)]
pos4 = [('-a/2-(DD-a-d/2)/2/N3*'+str(i), 'L1+p*'+str(i), 0) for i in range(1,N3)]
pos5 = [('a/2+(DD-a-d/2)/2/N3*'+str(i), 'L1+p*'+str(i), 0) for i in range(1,N3)]
...
for i in range(len(pos_x)):
    CreateCylinder(pos_x[i], r, h, name[i], material,WhichAxis = 'X')
